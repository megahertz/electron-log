...
var electronAppName;
try {
  electronAppName = require('electron').app.getName();
  if (electronAppName === 'Electron') {
...
function fileTransportFactory(electronLog) {
  ...
  function init(transp) {
    ...
    if (!transp.file) {
      var appName = transp.appName || electronAppName;
      transp.file = findLogPath(appName, transp.fileName);
    }
